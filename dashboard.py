...
with tab_growth:
    ...
    with col_vis2:
        ...
        # 차트 위에 " 몇 배" 인지 강조 주석 추가
        fig_prod_comp.add_annotation(
            x='인플루언서(킹댕즈)',
            y=summary_stats[summary_stats['그룹'] == '인플루언서(킹댕즈)']['셀러 1인당 평균 매출'].values[0],
            text=f"<b>약 {ratio:.0f}배 차이</b>",
            showarrow=True, arrowhead=2, ay=-40,
            bgcolor="white", bordercolor="#FF4B4B"
        )
        
        fig_prod_comp.update_layout(yaxis_title="평균 매출액 (원)", showlegend=False, height=450)
    ...
    **💡 분석 결과 및 전략적 시사점**
    ...
    with tab_strat2:
        st.write("**📈 일반 셀러 성장 전략 (Big Power 셀러 육성 로드맵)**")
        st.markdown("""
        일반 셀러가 인플루언서급 파급력을 갖지 않더라도, 단계별 육성을 통해 **'빅파워 셀러'**군단으로 성장시키기 위한 정교한 지원 체계를 구축합니다.
        """)
        
        # 1. 셀러 등급제 및 단계별 보상 체계
        st.write("#### 🥇 셀러 등급 구분을 통한 맞춤 동칭")
        cols_tier = st.columns(3)
        cols_tier[0].metric("Seed 셀러", "신입", "입점 캠페인 대상")
        cols_tier[1].metric("Power 셀러", "월 50회↑", "마케팅 지원")
        cols_tier[2].metric("Big Power", "월 200회↑", "수수료 혜택")
        
        st.markdown("---")
        
        col_gs1, col_gs2 = st.columns(2)
        with col_gs1:
            st.write("**✅ [즉각 도입] 매출 극대화 전략**")
            st.warning("""
            **1. 객단가(AOV) 확대**
            - **프리미엄 단독 기획전**: 고단가 품목(특대과, 선물세트) 일반 셀러 전용 노출.
            - **VIP 선물하기 전용관**: 일반 셀러의 단골 고객들을 위한 '프리미엄 선물하기' UI 제공.
            
            **2. 재구매(Retention) 증가**
            - **베스트 상품 리뷰 이벤트**: 일반 셀러 상품 리뷰 작성 시 포인트 2배 적립.
            - **구매 후 리마인드 쿠폰**: 첫 구매 후 1개월 내 재구매 시 사용 가능한 '자동 발송 쿠폰' 시스템 도입.
            """)
            
            st.write("**✅ [추천] 신규 셀러 확보**")
            st.success("""
            - **신규 입점 캠페인**: 초기 3개월 수수료 면제 또는 홍보 집행권 부여.
            - **셀러 추천 보상 확대**: 기존 셀러가 신규 셀러 추천 시, 양측 모두에게 판매 장려금 지급.
            """)
            
        with col_gs2:
            st.write("**✅ [지원] 기초 역량 강화**")
            st.info("""
            **1. 콘텐츠 및 광고 지원**
            - **상세페이지 진단**: 전문 MD의 썸네일/상세페이지 무료 UI/UX 유선 가이드.
            - **검색 광고 소액 지원**: 월 목표 달성 시 네이버/카카오 검색 광고 포인트 페이백.
            
            **2. 이탈 방어(Anti-Churn)**
            - **2개월 미활동 자동 알림**: 활동 중단 조짐 시 '복귀 독려 알림톡' 발송.
            - **휴면 복귀 쿠폰**: 3개월 이상 미활동 셀러 복귀 시 첫 거래 축하 쿠폰 지급.
            - **수수료 정책 점검**: 등급별 상향 수수료 인하 정책을 통해 장기 활동 유인 제공.
            """)
