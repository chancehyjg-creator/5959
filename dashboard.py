...
# 3. 탭 구성 (EDA 및 상세 분석)
# ----------------------------------------------------------------
tab1, tab2, tab_prod, tab_funnel, tab_time, tab_grade, tab3, tab4, tab5, tab6 = st.tabs([
    "📈 매출 & 채널", "📊 셀러 & 로열티", "📦 상품 페이지 분석", "🔁 재구매 퍼널", "⏰ 구매 시점 분석", "� 등급별 분석", "�🗺️ 지역별 분석", "🔍 경로 상세분석", "🎯 마케팅 전략", "📋 전체데이터"
])
...
# --- 탭: 등급별 분석 (신규) ---
with tab_grade:
    st.subheader("💎 상품 등급별(프리미엄 vs 일반) 구매 패턴 심층 분석")
    st.markdown("""
    업데이트된 **상품성등급_그룹** 기준을 바탕으로 고단가 프리미엄 고객과 실속형 일반 고객의 행동 차이를 분석합니다.
    """)

    # 1. 핵심 지표 비교
    col_g1, col_g2, col_g3 = st.columns(3)
    grade_metrics = f_df.groupby('상품성등급_그룹').agg({
        '실결제 금액': ['sum', 'mean'],
        '주문번호': 'count',
        '재구매여부': 'mean'
    }).reset_index()
    grade_metrics.columns = ['등급', '총매출', '평균객단가', '주문건수', '재구매비중']

    with col_g1:
        st.write("**💰 등급별 매출 기여도**")
        fig_g_rev = px.pie(grade_metrics, values='총매출', names='등급', hole=0.4,
                            color_discrete_map={'프리미엄': '#FFD700', '일반': '#C0C0C0'})
        st.plotly_chart(fig_g_rev, use_container_width=True)
    with col_g2:
        st.write("**📈 평균 객단가(AOV) 비교**")
        fig_g_aov = px.bar(grade_metrics, x='등급', y='평균객단가', color='등급',
                            color_discrete_map={'프리미엄': '#FFD700', '일반': '#C0C0C0'}, text_auto='.0f')
        st.plotly_chart(fig_g_aov, use_container_width=True)
    with col_g3:
        st.write("**🔄 재구매 비중 비교**")
        fig_g_rep = px.bar(grade_metrics, x='등급', y='재구매비중', color='등급',
                            color_discrete_map={'프리미엄': '#FFD700', '일반': '#C0C0C0'}, text_auto='.1%')
        st.plotly_chart(fig_g_rep, use_container_width=True)

    st.markdown("---")

    # 2. 채널별 선호 패턴 (유입 경로 클러스터링)
    st.write("#### 🗺️ 등급별 선호 유입 경로 (Channel Preference)")
    channel_grade = f_df.groupby(['주문경로', '상품성등급_그룹']).size().unstack(fill_value=0).reset_index()
    channel_grade['프리미엄비중'] = (channel_grade['프리미엄'] / (channel_grade['프리미엄'] + channel_grade['일반']) * 100).round(1)
    
    fig_g_chan = px.bar(channel_grade.sort_values('프리미엄', ascending=False), 
                        x='주문경로', y=['일반', '프리미엄'], 
                        title="주문 경로별 등급 구성 (건수 기준)",
                        barmode='stack',
                        color_discrete_map={'프리미엄': '#FFD700', '일반': '#C0C0C0'})
    st.plotly_chart(fig_g_chan, use_container_width=True)

    # 3. 구매 시간대 패턴 (Time Pattern Clustering)
    st.write("#### ⏰ 등급별 구매 프라임 타임 비교")
    f_df['hour'] = f_df['주문일'].dt.hour
    time_grade = f_df.groupby(['hour', '상품성등급_그룹']).size().unstack(fill_value=0).reset_index()
    
    # 시간대별 비중 정규화 (추세 비교용)
    time_grade['일반_비중'] = time_grade['일반'] / time_grade['일반'].sum()
    time_grade['프리미엄_비중'] = time_grade['프리미엄'] / time_grade['프리미엄'].sum()
    
    fig_g_time = go.Figure()
    fig_g_time.add_trace(go.Scatter(x=time_grade['hour'], y=time_grade['프리미엄_비중'], name='프리미엄', line=dict(color='#FFD700', width=4)))
    fig_g_time.add_trace(go.Scatter(x=time_grade['hour'], y=time_grade['일반_비중'], name='일반', line=dict(color='#C0C0C0', width=2, dash='dot')))
    fig_g_time.update_layout(title="시간대별 주문 비중 추이 비교", xaxis_title="시간(Hour)", yaxis_title="주문 비중")
    st.plotly_chart(fig_g_time, use_container_width=True)

    st.markdown("---")
    
    # 4. 마케팅 인사이트 도출
    st.success("""
    **🎯 분석 기반 등급별 마케팅 전략 (Insight Clustering)**
    
    **1. 프리미엄 고객군 (Premium Segment)**
    - **핵심 패턴**: 객단가가 일반 대비 월등히 높으며, 특정 '만감류' 시즌이나 '선물 키워드'에 민감하게 반응합니다.
    - **전략**: 가격보다는 **'희소성'**과 **'품질 보증'**이 중요합니다. "VIP 전용 한정수량 로얄과" 와 같은 프리미엄 타겟 알림톡을 발송하세요.
    - **채널**: 프리미엄 비중이 높은 채널(예: 카카오톡/인스타 등 시각 매체)에 고화질 이미지 광고를 집중 배치합니다.
    
    **2. 일반 고객군 (General Segment)**
    - **핵심 패턴**: 주문 건수가 압도적이며, 요일/시간별 피크 타임에 안정적인 수요를 보입니다. 가성비 높은 '대용량'이나 '실속형 혼합과'를 주로 구매합니다.
    - **전략**: **'가격 경쟁력'**과 **'편의성'**이 최우선입니다. 주문 피크 타임 직전에 "오늘의 특가" 푸시를 통해 대량 유입을 유도하세요.
    - **리텐션**: 일반 고객의 재구매율을 높이기 위해 "가정용 단골 할인권" 등을 통해 반복 구매를 유도하는 것이 매출 방어에 유리합니다.
    """)
with tab4:
